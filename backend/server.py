...
async def process_with_gemini_free(question: str, conversation_mode: str = 'normal', file_content: str = None, file_name: str = None) -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-002:generateContent?key={GEMINI_API_KEY}",
                headers=headers,
                json=payload,
                timeout=30.0
            )
...
async def google_auth():
    redirect_url = "https://rag-websearch.preview.emergentagent.com/dashboard"
